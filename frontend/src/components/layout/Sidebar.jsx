...
import {
  ...
  BookOpen, Building2, Settings, Package, List, Download, BarChart2, LogOut,
} from 'lucide-react'
  ...
  return (
    <aside className="hidden lg:flex w-56 shrink-0 h-screen sticky top-0 flex-col bg-[#161b22] border-r border-[rgba(48,54,61,0.8)]">
      ...
      <div className="px-5 py-5 border-b border-[rgba(48,54,61,0.8)]">
        ...
        <div className="text-xs text-[#3fb950] mt-1 font-medium truncate" title={storeName}>
          {storeName || 'Store'}
        </div>
      </div>
      ...
      <div className="px-3 pb-3 space-y-2">
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-medium text-[#8b949e] hover:text-red-400 hover:bg-red-900/10 transition-colors"
        >
          <LogOut size={15} />
          Sign Out
        </button>
        <div className="px-3 text-xs text-[#8b949e]">v2.1.0</div>
      </div>
    </aside>
  )
}
