...
export default function InventoryPage() {
  ...
  return (
    <Layout title="Inventory">
      {alerts?.length > 0 && (
        <div className="glass-card p-4 mb-6 border border-red-800/50 bg-red-900/10">
          <div className="flex items-center gap-2 mb-3">
            <AlertTriangle size={18} className="text-red-400" />
            <h2 className="text-red-400 font-semibold">Low Stock Alerts ({alerts.length})</h2>
          </div>
          <div className="space-y-1">
            {alerts.map((a) => (
              <p key={a.product_id} className="text-[#e6edf3] text-sm">
                <span className="font-medium">{a.name}</span>
                <span className="text-red-400 ml-2 font-mono">{a.current_stock}/{a.reorder_level}</span>
              </p>
            ))}
          </div>
        </div>
      ...
      <div className="glass-card overflow-hidden">
        <div className="px-5 py-4 border-b border-[rgba(48,54,61,0.8)]">
          <h2 className="text-[#e6edf3] font-semibold">All Products — Update Stock</h2>
        </div>
            ...
            {products?.map((p) => {
              ...
              return (
                <tr key={p.id} className={`border-b border-[rgba(48,54,61,0.3)] ${isLow ? 'bg-red-900/10' : ''}`}>
                  ...
                  <td className="px-3 py-3 text-center font-mono text-[#8b949e]">
                    {p.reorder_level}
                  </td>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        disabled={updateMap[p.id] == null || updateMutation.isPending}
                      >
                        Save
                      </button>
                    </div>
